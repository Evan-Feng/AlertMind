...
### 3.5 子代理偏离 prompt 的透明度要求（Deviation Transparency）

**当 subagent 的交付偏离派发 prompt 的任何具体指示（样例代码、命名、顺序、文件路径、数值常量等）时，回执中必须明确声明：**

> "我改动了 [具体点] 因为 [原因]，偏离派发 prompt 的 [原内容]。"

禁止伪装成"本来就是这样"或"这是自然的选择"。

主 Agent 收到回执后必须：

1. **Read 实际文件验证** subagent 报告的真实性（§2.4 Read-Before-Claim 的延伸：不信任回执的文字描述，只信仓库里的字节）
2. 把偏离的每一项独立列出，**供用户裁决**
3. **不要替用户"合理化"偏离**（如"这应该没问题吧" / "这符合生态惯例"）——合理化的判断权归用户

理由：subagent 的主动判断经常是对的（例如 `__all__` 用字母序防御 ruff RUF022），但"伪装成默认"会掩盖决策过程，未来 git blame 和 review 失去轨迹。显式留痕的代价 = 1 行回执 + 1 个 flag；代价几乎为零，价值是所有未来 review 者能还原当时的决策链路。

与 §2.4 / §3.4 互为三位一体：

- §2.4：**观察前**不要臆测（Read-Before-Claim）
- §3.4：**派发前**不要授权无观察变更（Pre-Dispatch Verification）
- §3.5：**派发后**不要让偏离被伪装成默认（Deviation Transparency）
